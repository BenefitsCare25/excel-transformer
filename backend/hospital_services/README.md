...
- `checkpoints.py` saves each finished page (redacted image + extracted fields) under `queue/<run_id>.work/`. A restarted worker resumes after the last finished page rather than repeating the document; the folder is removed once the result is saved. Gunicorn request-count recycling (`max_requests`) is disabled so progress polling cannot restart the OCR worker mid-document.
